...
The verified harnesses are `claude`, `codex`, `opencode`, `pi`, `pi-signed`, `grok`, `kimi`, and `cursor`, plus `muse` and `prime` for crewmates and scouts only; never dispatch on an unverified adapter.
