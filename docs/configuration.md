...
prime (Prime Agent) is likewise verified for crewmate and scout launches ONLY, and `fm-spawn.sh` refuses it for a secondmate because no Prime primary integration exists. Prime is also herdr-only: `fm-spawn.sh` refuses it on every other backend before it creates an endpoint. [`docs/verification/prime.md`](verification/prime.md) owns that evidence.
