...
prime (Prime Agent) is likewise verified for crewmate and scout launches ONLY, and `fm-spawn.sh` refuses it for a secondmate because no Prime primary integration exists; [`docs/verification/prime.md`](verification/prime.md) owns that evidence, including its Herdr-only backend coverage.
