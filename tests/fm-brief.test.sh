...
test_ship_build_methodology() {
  ...
  for mode in no-mistakes direct-PR local-only; do
    ...
    # shellcheck disable=SC2016  # single quotes are deliberate: the backticks and dollar-must-stay-literal in the needle
assert_contains "$section" 'Only when this brief asks, run `/greploop` on the opened PR' "$mode: PR review must be conditional"
    assert_contains "$section" "Completion follows the Definition of done above" "$mode: missing completion precedence"
    assert_contains "$section" "report done when CI first turns green for no-mistakes" "$mode: missing CI completion boundary"
