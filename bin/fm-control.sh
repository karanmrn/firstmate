...
resolve_relaunch_profile() {
  ...
  backend_reason=$(fm_control_harness_supports_backend "$TARGET_HARNESS" "$BACKEND") \
    || die "$backend_reason, so relaunching $ID onto it would stop the running agent for a launch that must be refused; choose an adapter verified for the $BACKEND backend"
